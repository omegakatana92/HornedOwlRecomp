...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\hornedOwl.ps1
...
#     %APPDATA%\HornedOwlRecomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Project_Horned_Owl_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'HornedOwlRecomp'
...
$form.Text = 'Project Horned Owl Launcher'
...
$lblInfo.Text = "Select your legally obtained Project Horned Owl disc image (.cue).
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Project Horned Owl disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Project Horned Owl Launcher', 'OK', 'Warning') | Out-Null
}
