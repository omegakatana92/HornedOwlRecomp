...
A modern fan-made recompilation project for **Project Horned Owl**, originally released for the **Sony PlayStation**.
...
To play, you must provide your **own legally obtained copy** of *Project Horned Owl*.
...
* `launcher\hornedOwl.bat` â€” double-click to start.
* `launcher\hornedOwl.ps1` â€” the actual PowerShell + WinForms GUI.

The launcher lets you:

1. Pick your legally obtained Project Horned Owl `.cue` file (Browse...).
...
`%APPDATA%\%HornedOwlRecomp\launcher-config.json` and re-loaded the next time
the launcher starts.
...
```bash
git clone https://github.com/omegakatana92/HornedOwlRecomp.git
cd HornedOwlRecomp
```
...
**Project Horned Owl**, its characters, names, artwork, music, trademarks, and other intellectual property remain the property of their respective copyright and trademark holders.
